...
import {
  getLessonIndexedBd,
  getRulesStatusIndexedBD,
} from "../../common/servicesIndexdBD/lessonsIndexedBD.js";
...
const TaskListPage = () => {
  ...
  if (!currentLesson.tasks) {
    return (
      <div className="container_errTest">
        ...
        {readTheRules === "successfully" ? (
          currentLesson.tasks.map((task) => (
            <Link
              ...
              className={`taskList-task-card ${
                +activeLesson === task.id ? "active-task-card" : ""
              } `}
              key={task.id}
            >
          ...
          <div className="taskList-messageOfRules">
            <p className="taskList-messageOfRules-text">
              Прежде чем приступить к решению задач, обязательно изучи правила
              пользования практикумом! Это поможет тебе обратить внимание на
              важные моменты и избежать ошибок. После того, как ознакомишься с
              правилами, задачи станут доступны для решения.
            </p>
            <button className="taskList-messageOfRules-btn">
              <Link
                to="/practicum-rules"
                className="taskList-messageOfRules-link"
              >
